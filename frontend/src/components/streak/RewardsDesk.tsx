...
export default function RewardsDesk({ coins, hasBooked, onUpdateCoins, onSelectBadge, onToast }: Props) {
  ...
  const scrollPeek = 28;
  const trophyCardWidth = Math.floor((screenWidth - space[4] * 4 - space[2] - scrollPeek) / 2);
  const trophySnap = trophyCardWidth + space[2];
...
const styles = StyleSheet.create({
  ...
  sub: { ...typeScale.bodySm, color: colors.text.secondary, marginTop: space[1], marginBottom: space[3] },
  tabs: {
    flexDirection: 'row',
  ...
  tabBtn: { flex: 1, paddingVertical: 6, borderRadius: radius.sm, alignItems: 'center', minHeight: 32 },
  ...
  badgeScroll: { gap: space[2], paddingRight: space[1] },
  scrollHint: {
    position: 'absolute',
    top: 0,
    right: 0,
  ...
  lockDot: { position: 'absolute', top: space[1], right: space[1] },
  ...
  shareEmoji: { fontSize: 24, marginBottom: space[1] },
  ...
  freezeRow: { flexDirection: 'row', alignItems: 'center', gap: space[2] },
  freezeIcon: {
    width: 32,
    height: 32,
    borderRadius: radius.sm,
  ...
  freezeDesc: { ...typeScale.caption, color: colors.text.secondary, marginTop: 2, lineHeight: 15 },
  activeChip: { backgroundColor: '#BAE6FD', paddingHorizontal: space[2], paddingVertical: 4, borderRadius: radius.sm },
